...
const CONFIG_DIR: &str = ".scrapeless-scraping-browser";
const CONFIG_FILENAME: &str = "config.json";
const PROJECT_CONFIG_FILENAME: &str = "scrapeless-scraping-browser.json";
...
pub fn load_config(args: &[String]) -> Result<Config, String> {
    let explicit = extract_config_path(args)
        .map(|p| ("--config", p))
        .or_else(|| {
            env::var("SCRAPELESS_BROWSER_CONFIG")
                .ok()
                .map(|p| ("SCRAPELESS_BROWSER_CONFIG", Some(p)))
        });
...
pub fn parse_flags(args: &[String]) -> Flags {
    ...
    let extensions_env = env::var("SCRAPELESS_BROWSER_EXTENSIONS")
        .ok()
        .map(|s| {
            s.split(',')
    ...
    let mut flags = Flags {
        json: env_var_is_truthy("SCRAPELESS_BROWSER_JSON") || config.json.unwrap_or(false),
        full: env_var_is_truthy("SCRAPELESS_BROWSER_FULL") || config.full.unwrap_or(false),
        headed: env_var_is_truthy("SCRAPELESS_BROWSER_HEADED") || config.headed.unwrap_or(false),
        debug: env_var_is_truthy("SCRAPELESS_BROWSER_DEBUG") || config.debug.unwrap_or(false),
        session: env::var("SCRAPELESS_BROWSER_SESSION")
            .ok()
            .or(config.session)
            .unwrap_or_else(|| "default".to_string()),
        headers: config.headers,
        executable_path: env::var("SCRAPELESS_BROWSER_EXECUTABLE_PATH")
            .ok()
            .or(config.executable_path),
        extensions,
        profile: env::var("SCRAPELESS_BROWSER_PROFILE").ok().or(config.profile),
        state: env::var("SCRAPELESS_BROWSER_STATE").ok().or(config.state),
        proxy: env::var("SCRAPELESS_BROWSER_PROXY").ok().or(config.proxy),
        proxy_bypass: env::var("SCRAPELESS_BROWSER_PROXY_BYPASS")
            .ok()
            .or(config.proxy_bypass),
        args: env::var("SCRAPELESS_BROWSER_ARGS").ok().or(config.args),
        user_agent: env::var("SCRAPELESS_BROWSER_USER_AGENT")
            .ok()
            .or(config.user_agent),
        ignore_https_errors: env_var_is_truthy("SCRAPELESS_BROWSER_IGNORE_HTTPS_ERRORS")
            || config.ignore_https_errors.unwrap_or(false),
        allow_file_access: env_var_is_truthy("SCRAPELESS_BROWSER_ALLOW_FILE_ACCESS")
            || config.allow_file_access.unwrap_or(false),
        device: env::var("SCRAPELESS_BROWSER_IOS_DEVICE").ok().or(config.device),
        session_name: env::var("SCRAPELESS_BROWSER_SESSION_NAME")
            .ok()
            .or(config.session_name),
        session_id: env::var("SCRAPELESS_SESSION_ID")
            .ok()
            .or(config.session_id),
        annotate: env_var_is_truthy("SCRAPELESS_BROWSER_ANNOTATE") || config.annotate.unwrap_or(false),
        color_scheme: env::var("SCRAPELESS_BROWSER_COLOR_SCHEME")
            .ok()
            .or(config.color_scheme),
        download_path: env::var("SCRAPELESS_BROWSER_DOWNLOAD_PATH")
            .ok()
            .or(config.download_path),
        content_boundaries: env_var_is_truthy("SCRAPELESS_BROWSER_CONTENT_BOUNDARIES")
            || config.content_boundaries.unwrap_or(false),
        max_output: env::var("SCRAPELESS_BROWSER_MAX_OUTPUT")
            .ok()
            .and_then(|s| s.parse().ok())
            .or(config.max_output),
        allowed_domains: env::var("SCRAPELESS_BROWSER_ALLOWED_DOMAINS")
            .ok()
            .map(|s| {
                s.split(',')
        ...
        action_policy: env::var("SCRAPELESS_BROWSER_ACTION_POLICY")
            .ok()
            .or(config.action_policy),
        confirm_actions: env::var("SCRAPELESS_BROWSER_CONFIRM_ACTIONS")
            .ok()
            .or(config.confirm_actions),
        confirm_interactive: env_var_is_truthy("SCRAPELESS_BROWSER_CONFIRM_INTERACTIVE")
            || config.confirm_interactive.unwrap_or(false),
        native: env_var_is_truthy("SCRAPELESS_BROWSER_NATIVE") || config.native.unwrap_or(false),
        engine: env::var("SCRAPELESS_BROWSER_ENGINE").ok().or(config.engine),
        api_version: env::var("SCRAPELESS_API_VERSION").ok().or(config.api_version),
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--api-version" => {
                if let Some(s) = args.get(i + 1) {
                    flags.api_version = Some(s.clone());
                    i += 1;
                }
            }
            "--config" => {
