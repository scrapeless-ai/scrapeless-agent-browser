...
pub async fn save_state(
    ...
    if let Ok(key) = std::env::var("SCRAPELESS_BROWSER_ENCRYPTION_KEY") {
...
pub async fn load_state(client: &CdpClient, session_id: &str, path: &str) -> Result<(), String> {
    let json_str = if path.ends_with(".enc") {
        let key = std::env::var("SCRAPELESS_BROWSER_ENCRYPTION_KEY").map_err(|_| {
            "Encrypted state file requires SCRAPELESS_BROWSER_ENCRYPTION_KEY".to_string()
        })?;
        let data =
...
pub fn get_sessions_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".scrapeless-scraping-browser").join("sessions")
    } else {
