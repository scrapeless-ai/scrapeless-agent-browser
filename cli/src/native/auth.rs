...
fn get_auth_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".scrapeless-scraping-browser").join("auth")
    } else {
...
const ENCRYPTION_KEY_ENV: &str = "SCRAPELESS_BROWSER_ENCRYPTION_KEY";
const KEY_FILE_NAME: &str = ".encryption-key";

fn get_agent_browser_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".scrapeless-scraping-browser")
    } else {
...
/// Read the encryption key from SCRAPELESS_BROWSER_ENCRYPTION_KEY env var or
/// ~/.scrapeless-scraping-browser/.encryption-key file (matching the Node.js implementation).
