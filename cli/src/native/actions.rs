...
impl DaemonState {
    pub fn new() -> Self {
        Self {
            ...
            ref_map: RefMap::new(),
            domain_filter: env::var("SCRAPELESS_BROWSER_ALLOWED_DOMAINS")
                .ok()
                .filter(|s| !s.is_empty())
                .map(|s| DomainFilter::new(&s)),
            event_tracker: EventTracker::new(),
            session_name: env::var("SCRAPELESS_BROWSER_SESSION_NAME").ok(),
            session_id: env::var("SCRAPELESS_BROWSER_SESSION").unwrap_or_else(|_| "default".to_string()),
...
pub async fn execute_command(cmd: &Value, state: &mut DaemonState) -> Value {
    ...
    // Check SCRAPELESS_BROWSER_CONFIRM_ACTIONS (category-based, independent of policy file)
...
async fn auto_launch(state: &mut DaemonState) -> Result<(), String> {
    ...
    let engine = env::var("SCRAPELESS_BROWSER_ENGINE").ok();

    if let Ok(cdp) = env::var("SCRAPELESS_BROWSER_CDP") {
    ...
    if env::var("SCRAPELESS_BROWSER_AUTO_CONNECT").is_ok() {
...
fn launch_options_from_env() -> LaunchOptions {
    let headed = env::var("SCRAPELESS_BROWSER_HEADED")
        .map(|v| v == "1" || v == "true")
        .unwrap_or(false);

    let extensions: Option<Vec<String>> = env::var("SCRAPELESS_BROWSER_EXTENSIONS").ok().map(|v| {
        v.split([',', '\n'])
    ...
    LaunchOptions {
        headless: !headed,
        executable_path: env::var("SCRAPELESS_BROWSER_EXECUTABLE_PATH").ok(),
        proxy: env::var("SCRAPELESS_BROWSER_PROXY").ok(),
        proxy_bypass: env::var("SCRAPELESS_BROWSER_PROXY_BYPASS").ok(),
        profile: env::var("SCRAPELESS_BROWSER_PROFILE").ok(),
        allow_file_access: env::var("SCRAPELESS_BROWSER_ALLOW_FILE_ACCESS")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false),
        args: env::var("SCRAPELESS_BROWSER_ARGS")
            .map(|v| {
                v.split([',', '\n'])
                    .map(|s| s.trim().to_string())
                    .filter(|s| !s.is_empty())
            ...
            .unwrap_or_default(),
        extensions,
        storage_state: env::var("SCRAPELESS_BROWSER_STATE").ok(),
        user_agent: env::var("SCRAPELESS_BROWSER_USER_AGENT").ok(),
        ignore_https_errors: env::var("SCRAPELESS_BROWSER_IGNORE_HTTPS_ERRORS")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false),
        color_scheme: env::var("SCRAPELESS_BROWSER_COLOR_SCHEME").ok(),
        download_path: env::var("SCRAPELESS_BROWSER_DOWNLOAD_PATH").ok(),
    }
}

fn daemon_state_from_env(state: &mut DaemonState) {
    if let Ok(name) = env::var("SCRAPELESS_BROWSER_SESSION_NAME") {
        if !name.is_empty() {
            state.session_name = Some(name);
        }
    }
    if let Ok(domains) = env::var("SCRAPELESS_BROWSER_ALLOWED_DOMAINS") {
        if !domains.is_empty() {
...
async fn handle_launch(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let executable_path: Option<String> = cmd
        ...
        .and_then(|v| v.as_str())
        .map(String::from)
        .or_else(|| std::env::var("SCRAPELESS_BROWSER_EXECUTABLE_PATH").ok());
    ...
    let engine = cmd
        ...
        .and_then(|v| v.as_str())
        .map(String::from)
        .or_else(|| env::var("SCRAPELESS_BROWSER_ENGINE").ok());
    ...
    let options = LaunchOptions {
        ...
        executable_path: cmd
            ...
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .or_else(|| env::var("SCRAPELESS_BROWSER_EXECUTABLE_PATH").ok()),
    ...
    let save_path = match path {
        ...
        None => {
            let dir = dirs::home_dir()
                ...
                .join(".scrapeless-scraping-browser")
                .join("tmp")
                .join("pdfs");
...
mod tests {
    ...
    fn test_launch_options_from_env_headed_flag() {
        let _guard = EnvGuard::new(&["SCRAPELESS_BROWSER_HEADED"]);
        _guard.set("SCRAPELESS_BROWSER_HEADED", "1");
        let opts = launch_options_from_env();
        assert!(
            !opts.headless,
            "SCRAPELESS_BROWSER_HEADED=1 should set headless=false"
        );
    }
