...
/// Confirmation categories parsed from SCRAPELESS_BROWSER_CONFIRM_ACTIONS.
#[derive(Debug, Clone)]
...
impl ConfirmActions {
    pub fn from_env() -> Option<Self> {
        let val = env::var("SCRAPELESS_BROWSER_CONFIRM_ACTIONS").ok()?;
        if val.is_empty() {
            return None;
        }
...
impl ActionPolicy {
    ...
    /// Load policy if SCRAPELESS_BROWSER_ACTION_POLICY env var is set.
    /// Falls back to SCRAPELESS_BROWSER_POLICY for backwards compatibility.
    pub fn load_if_exists() -> Option<Self> {
        let path = env::var("SCRAPELESS_BROWSER_ACTION_POLICY")
            .or_else(|_| env::var("SCRAPELESS_BROWSER_POLICY"))
            .ok()?;
        Self::load(&path).ok()
    }
...
mod tests {
    ...
    fn test_confirm_actions_from_env() {
        let _guard = EnvGuard::new(&["SCRAPELESS_BROWSER_CONFIRM_ACTIONS"]);
        _guard.set("SCRAPELESS_BROWSER_CONFIRM_ACTIONS", "navigate,click,fill");
