...
// Scrapeless session management schemas
const scrapelessCreateSchema = baseCommandSchema.extend({
  action: z.literal('scrapeless_create'),
  sessionName: z.string().optional(),
...
const scrapelessSessionsSchema = baseCommandSchema.extend({
  action: z.literal('scrapeless_sessions'),
});

const scrapelessStopSchema = baseCommandSchema.extend({
  action: z.literal('scrapeless_stop'),
  taskId: z.string().min(1),
});

const scrapelessStopAllSchema = baseCommandSchema.extend({
  action: z.literal('scrapeless_stop_all'),
});

const scrapelessLiveSchema = baseCommandSchema.extend({
  action: z.literal('scrapeless_live'),
...
const commandSchema = z.discriminatedUnion('action', [
  ...
  authDeleteSchema,
  authShowSchema,
  scrapelessCreateSchema,
  scrapelessSessionsSchema,
  scrapelessStopSchema,
  scrapelessStopAllSchema,
  scrapelessLiveSchema,
]);
