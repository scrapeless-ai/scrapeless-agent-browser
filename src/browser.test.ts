...
describe('BrowserManager', () => {
  ...
  beforeAll(async () => {
    // Set a mock API key for testing (required by API client initialization)
    process.env.SCRAPELESS_API_KEY = 'sk_test_mock_key_for_testing';

    // Skip actual browser launch since it requires real Scrapeless connection
    // These tests focus on the BrowserManager class structure and methods
    browser = new BrowserManager();

    // Mock the launch to skip actual Scrapeless connection
    vi.spyOn(browser, 'connectToScrapeless' as any).mockResolvedValue(undefined);
  ...
  describe('annotated screenshots', () => {
    ...
    it('should clean up overlay from DOM after screenshot', async () => {
      ...
      const overlay = await page.$('#__scrapeless_browser_annotations__');
