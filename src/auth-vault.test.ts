...
describe('auth-vault', () => {
  beforeEach(() => {
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scrapeless-browser-auth-test-'));
    delete process.env.SCRAPELESS_BROWSER_ENCRYPTION_KEY;
  });
  ...
  function cleanAuthDir() {
    const authDir = path.join(tempHome, '.scrapeless-scraping-browser', 'auth');
  ...
  describe('auto-generated encryption key', () => {
    it('should auto-create key file and encrypt profile when no env var is set', () => {
      ...
      const keyFilePath = path.join(tempHome, '.scrapeless-scraping-browser', '.encryption-key');
