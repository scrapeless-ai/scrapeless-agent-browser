...
describe('getSocketDir', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.SCRAPELESS_BROWSER_SOCKET_DIR;
  ...
  describe('SCRAPELESS_BROWSER_SOCKET_DIR', () => {
    it.skip('should use custom path when set', () => {
      // This test is skipped because SCRAPELESS_BROWSER_SOCKET_DIR is not implemented
      // getSocketDir() always returns getAppDir()
      process.env.SCRAPELESS_BROWSER_SOCKET_DIR = '/custom/socket/path';
      expect(getSocketDir()).toBe('/custom/socket/path');
    });

    it('should ignore empty string', () => {
      process.env.SCRAPELESS_BROWSER_SOCKET_DIR = '';
      const result = getSocketDir();
      expect(result).toContain('.scrapeless-scraping-browser');
    });

    it.skip('should take priority over XDG_RUNTIME_DIR', () => {
      // This test is skipped because SCRAPELESS_BROWSER_SOCKET_DIR is not implemented
      process.env.SCRAPELESS_BROWSER_SOCKET_DIR = '/custom/path';
  ...
  describe('XDG_RUNTIME_DIR', () => {
    it('should use when SCRAPELESS_BROWSER_SOCKET_DIR is not set', () => {
      process.env.XDG_RUNTIME_DIR = '/run/user/1000';
  ...
  describe('fallback', () => {
    it('should use home directory when env vars are not set', () => {
      const result = getSocketDir();
      const expected = path.join(os.homedir(), '.scrapeless-scraping-browser');
