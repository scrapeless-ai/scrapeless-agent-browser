...
// Import new Scrapeless API
import { ScrapelessClient } from './api/client.js';
import { ScrapelessBrowserAPI } from './api/browser.js';
import { ScrapelessAPIError } from './api/errors.js';
import type {
  Command,
  Response,
  NavigateCommand,
...
export function initActionPolicy(): void {
  const policyPath = process.env.SCRAPELESS_BROWSER_ACTION_POLICY;
  if (policyPath) {
    try {
  ...
  const confirmActionsEnv = process.env.SCRAPELESS_BROWSER_CONFIRM_ACTIONS;
...
async function dispatchAction(command: Command, browser: BrowserManager): Promise<Response> {
  switch (command.action) {
    ...
    case 'config_list':
      return handleConfigList(command);
    case 'config_remove':
      return handleConfigRemove(command);

    // === Scrapeless Session Management ===
    case 'scrapeless_create':
      return handleScrapelessCreate(command);
    case 'scrapeless_sessions':
      return handleScrapelessSessions(command);
    case 'scrapeless_stop':
      return handleScrapelessStop(command);
    case 'scrapeless_stop_all':
      return handleScrapelessStopAll(command);
    case 'scrapeless_live':
      return handleScrapelessLive(command);
...
const ANNOTATION_OVERLAY_ID = '__scrapeless_browser_annotations__';
...
function handleConfigSet(command: any): Response {
  try {
    setConfigValue(command.key, command.value);
    return successResponse(command.id, {
      message: `✓ Set ${command.key === 'apiKey' ? 'key' : command.key} = ${command.value}`,
    });
  } catch (error) {
    return errorResponse(command.id, `Failed to set config: ${error}`);
  }
}

function handleConfigGet(command: any): Response {
  try {
    const config = listConfig();
    const value = (config as any)[command.key];
    return successResponse(command.id, {
      key: command.key === 'apiKey' ? 'key' : command.key,
      value: value || '(not set)',
    });
  } catch (error) {
    return errorResponse(command.id, `Failed to get config: ${error}`);
  }
}

function handleConfigList(command: any): Response {
  try {
    const config = listConfig();
    ...
    for (const [k, v] of Object.entries(config)) {
      ...
      displayConfig[displayKey] = v;
    }

    return successResponse(command.id, { config: displayConfig });
  } catch (error) {
    return errorResponse(command.id, `Failed to list config: ${error}`);
  }
}

function handleConfigRemove(command: any): Response {
  try {
    removeConfigValue(command.key);
    return successResponse(command.id, {
      message: `✓ Removed ${command.key === 'apiKey' ? 'key' : command.key}`,
    });
  } catch (error) {
    return errorResponse(command.id, `Failed to remove config: ${error}`);
  }
}

// === Scrapeless Session Management Handlers ===

/**
 * Create a new Scrapeless browser session
 */
async function handleScrapelessCreate(
  command: Command & {
    sessionName?: string;
    sessionTtl?: number;
  ...
  if (!apiKey) {
    return errorResponse(
      command.id,
      'SCRAPELESS_API_KEY is required. Set it with: config set apiKey YOUR_KEY'
    );
  }

  try {
    const client = new ScrapelessClient({ apiKey });
    const browserAPI = new ScrapelessBrowserAPI(client);

    // Prepare session options
    const options: any = {};

    // Add query parameters
    if (command.sessionName) options.sessionName = command.sessionName;
    if (command.sessionTtl !== undefined) options.sessionTTL = command.sessionTtl;
    if (command.sessionRecording !== undefined) options.sessionRecording = command.sessionRecording;
    if (command.proxyCountry) options.proxyCountry = command.proxyCountry;
    if (command.proxyState) options.proxyState = command.proxyState;
    if (command.proxyCity) options.proxyCity = command.proxyCity;

    // Add body parameters
    if (command.userAgent) options.userAgent = command.userAgent;
    if (command.platform) options.platform = command.platform;
    if (command.screenWidth || command.screenHeight) {
      options.screen = {
        width: command.screenWidth || 1920,
        height: command.screenHeight || 1080,
      };
    }
    if (command.timezone || command.languages) {
      options.localization = {
        basedOnIP: false,
        timezone: command.timezone || 'America/New_York',
        languages: command.languages || ['en'],
      };
    }
    ...
    if (error instanceof ScrapelessAPIError) {
      return errorResponse(command.id, `API Error: ${error.message}`);
    }
    return errorResponse(
      command.id,
...
/**
 * List all running Scrapeless sessions
 */
async function handleScrapelessSessions(command: Command): Promise<Response> {
  const configJson = JSON.stringify(listConfig());
  console.log('[DEBUG] Config contents:', configJson);
  const apiKey = getApiKey();
  console.log(
    '[DEBUG] handleScrapelessSessions - apiKey:',
    apiKey ? `${apiKey.substring(0, 10)}...` : 'undefined/null'
  );

  if (!apiKey) {
    return errorResponse(
      command.id,
      'SCRAPELESS_API_KEY is required. Set it with: config set apiKey YOUR_KEY [handleScrapelessSessions]'
    );
  }

  try {
    console.log(
      '[DEBUG] Creating ScrapelessClient with apiKey:',
      apiKey ? `${apiKey.substring(0, 10)}...` : 'undefined/null'
    );
    const client = new ScrapelessClient({ apiKey });
    const browserAPI = new ScrapelessBrowserAPI(client);

    const sessions = await browserAPI.getRunningSessions();
...
/**
 * Stop a specific Scrapeless session
 */
async function handleScrapelessStop(command: Command & { taskId: string }): Promise<Response> {
  const apiKey = getApiKey();
  if (!apiKey) {
    return errorResponse(
      command.id,
  ...
  if (!command.taskId) {
    return errorResponse(command.id, 'taskId is required');
  }

  try {
    const client = new ScrapelessClient({ apiKey });

    // Use direct API call for stop session (not implemented in browser API yet)
    await client.request('GET', `/browser/stop/${command.taskId}`);

    return successResponse(command.id, {
      stopped: true,
      taskId: command.taskId,
      message: `Session ${command.taskId} stopped successfully`,
    });
  } catch (error) {
    if (error instanceof ScrapelessAPIError) {
      return errorResponse(command.id, `API Error: ${error.message}`);
...
/**
 * Stop all running Scrapeless sessions
 */
async function handleScrapelessStopAll(command: Command): Promise<Response> {
  const apiKey = getApiKey();
  if (!apiKey) {
    return errorResponse(
      command.id,
  ...
  try {
    ...
    // Use direct API call for stop all sessions
    await client.request('GET', '/browser/stopAll');

    return successResponse(command.id, {
      stopped: true,
      message: 'All sessions stopped successfully',
    });
  } catch (error) {
    if (error instanceof ScrapelessAPIError) {
      return errorResponse(command.id, `API Error: ${error.message}`);
    ...
    return errorResponse(
      command.id,
      `Failed to stop sessions: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}

/**
 * Get live preview URL for a Scrapeless session
 */
async function handleScrapelessLive(command: Command & { taskId?: string }): Promise<Response> {
  const apiKey = getApiKey();
  if (!apiKey) {
    return errorResponse(
      command.id,
  ...
  // If no taskId provided, try to get the latest running session
  if (!taskId) {
    try {
      const client = new ScrapelessClient({ apiKey });
      const browserAPI = new ScrapelessBrowserAPI(client);

      const latestSession = await browserAPI.getLatestSession();
      if (latestSession) {
        taskId = latestSession.taskId;
      }
    } catch {
      // Ignore errors, will return error below
    }

    if (!taskId) {
      return errorResponse(
        command.id,
        'No active session found. Provide a taskId or start a session first.'
      );
    }
  }
  ...
  try {
    ...
    const liveUrl = await browserAPI.getLiveUrl(taskId);

    return successResponse(command.id, {
      taskId,
      liveUrl,
      message: 'Live preview URL retrieved successfully',
    });
  } catch (error) {
    if (error instanceof ScrapelessAPIError) {
      return errorResponse(command.id, `API Error: ${error.message}`);
    ...
    return errorResponse(
      command.id,
      `Failed to get live URL: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
