...
export const ENCRYPTION_KEY_ENV = 'SCRAPELESS_BROWSER_ENCRYPTION_KEY';
...
export function getKeyFilePath(): string {
  return join(os.homedir(), '.scrapeless-scraping-browser', KEY_FILE_NAME);
}
 ...
 *
 * Checks (in order):
 * 1. SCRAPELESS_BROWSER_ENCRYPTION_KEY env var
 * 2. ~/.scrapeless-scraping-browser/.encryption-key file
 *
 ...
 * and writes it to ~/.scrapeless-scraping-browser/.encryption-key (mode 0600).
 */
export function ensureEncryptionKey(): Buffer {
  ...
  const dir = join(os.homedir(), '.scrapeless-scraping-browser');
  if (!existsSync(dir)) {
