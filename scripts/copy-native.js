...
const sourcePath = join(projectRoot, `cli/target/release/scrapeless-scraping-browser${sourceExt}`);
const binDir = join(projectRoot, 'bin');

// Determine platform suffix
const platformKey = `${platform()}-${arch()}`;
const ext = platform() === 'win32' ? '.exe' : '';
const targetName = `scrapeless-scraping-browser-${platformKey}${ext}`;
